...
object GatlingZeromqRelease {
  ...
  private def updateVersionInReadme: ReleaseStep = { s: State =>
    ...
    s.log.info(
      s"Replacing $currentVersionInReadme with $releaseVersion in ${readmeFile.name}")

    val newReadme =
      readme.replaceAll(Pattern.quote(currentVersionInReadme), releaseVersion)
